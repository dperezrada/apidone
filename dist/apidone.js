...
var BSON, Db, MONGODB_DBNAME, MONGODB_HOST, MONGODB_PORT, Mongo, PORT, Server, app, async, clear_response, clone, clone_request, configure_app, create_internal_url, db, express, formatErrorHandler, get_limit, get_offset, get_prefix_interal_url, get_sort, mongodb, prepare_db_filters, retrieve_collection, retrieve_subdomain, server, set_cors, _;
...
clone_request = function(request) {
  return {
    route: {
...
app.get("/*", function(request, response) {
  ...
  if (local_request.route.params[0] === "__resources") {
    return Mongo.get_collections(db, retrieve_subdomain(local_request), function(err, collections) {
      return response.send(collections);
    });
  } else {
    return db.collection(local_request.collection, function(err, collection) {
      ...
      if (local_request.route.params[0].indexOf("__resources") > 0) {
        ...
        return collection.findOne({
          ...
          if (result) {
            ...
            _ref = get_sort(local_request.query, filters), _sort_by = _ref[0], _sort_type = _ref[1];
            ...
            if (local_request.query._select_distinct) {
              ...
              return collection.distinct(local_request.query._select_distinct, filters, function(error, items) {
              ...
              return collection.find(filters, {}, function(error, cursor) {
                return cursor.sort([[_sort_by, _sort_type]]).limit(limit).skip(offset).toArray(function(err, items) {
                  ...
                  if (items === null || items.length === 0) {
                    if (local_request.query._default) {
                      to_return = local_request.query._default;
                    }
...
app.post("/*", function(request, response) {
  ...
  return async.waterfall([
    async.apply(Mongo.get_collection, db, local_request.collection), function(collection, callback) {
      return Mongo.insert(collection, local_request.body, function(err, inserted_docs) {
        var id, _id, _internal_url;
        _internal_url = _id = null;
        if (!err) {
          id = _id = inserted_docs[0]["_id"];
          if (local_request.body.id) {
            id = local_request.body.id;
          }
    ...
    async.apply(Mongo.get_collection, db, local_request.collection), function(collection, callback) {
      return collection.findOne({
        ...
        if (resource) {
          ...
          return collection.update({
            _internal_url: local_request.route.params[0]
          }, local_request.body, function(error, result) {
            return callback(error, resource["_internal_url"], resource["_id"], collection);
          });
        } else {
          return Mongo.insert(collection, local_request.body, function(err, inserted_docs) {
            return callback(error, local_request.route.params[0], inserted_docs[0]["_id"], collection);
          });
        }
      });
    }, Mongo.update_internal_url
...
app["delete"]("/*", function(request, response) {
  ...
  return db.collection(local_request.collection, function(err, collection) {
    ...
    if (local_request.query._remove === "all") {
      query = {
    ...
    return collection.remove({
      ...
      if (result) {
        response.statusCode = 204;
        if (local_request.query._remove === "all") {
          collection.count({}, function(error, items) {
            if (items === 0) {
