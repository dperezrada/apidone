...
var BSON, Db, MONGODB_DBNAME, MONGODB_HOST, MONGODB_PORT, Mongo, PORT, Server, app, async, clear_response, configure_app, create_internal_url, db, express, formatErrorHandler, get_prefix_interal_url, mongodb, prepare_db_filters, retrieve_collection, retrieve_subdomain, server, set_cors, _;
...
db.open(function(err, data) {
    ...
    if (process.env.MONGODB_USER) {
      data.authenticate(process.env.MONGODB_USER, process.env.MONGODB_PASSWORD, function(err2, data2) {
        if (!err2) {
          ...
          return console.log("Error: Invalid database user or password");
        }
      });
    } else {
      db_connected = true;
  ...
  if (db_connected) {
    ...
    return console.log("Error: Problem connecting to database");
  }
});
    ...
    return db.collection(request.collection, function(err, collection) {
      ...
      if (request.route.params[0].indexOf("__resources") > 0) {
        ...
        return collection.findOne({
          ...
          var filters, _sort_by, _sort_type;
          if (result) {
            ...
            if (request.query._select_distinct) {
              ...
              return collection.find(filters, {}, function(error, cursor) {
                return cursor.sort([[_sort_by, _sort_type]]).toArray(function(err, items) {
                  var i, to_return;
                  to_return = [];
