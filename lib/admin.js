...
var BSON, Db, MONGODB_DBNAME, MONGODB_HOST, MONGODB_PORT, Mongo, PORT, SALT, Server, app, async, check_subdomain, configure_app, crypto, db, encrypt_pass, express, formatErrorHandler, mongodb, server, set_cors, _;
            ...
            return check_subdomain(collection, request.body.subdomain, function(err, status) {
              if (err) {
                ...
                return Mongo.insert(collection, request.body, function(err, inserted_account) {
                  return callback(err, request.body.subdomain);
                });
              } else {
                return callback({
                  'code': 409,
