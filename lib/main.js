...
var BSON, Db, MONGODB_DBNAME, MONGODB_HOST, MONGODB_PORT, Mongo, PORT, Server, app, async, clear_response, configure_app, create_internal_url, db, express, formatErrorHandler, get_limit, get_prefix_interal_url, get_sort, mongodb, prepare_db_filters, retrieve_collection, retrieve_subdomain, server, set_cors, _;
    ...
    return db.collection(request.collection, function(err, collection) {
      ...
      if (request.route.params[0].indexOf("__resources") > 0) {
        ...
        return collection.findOne({
          ...
          var filters, limit, _ref, _sort_by, _sort_type;
          if (result) {
            ...
            if (request.query._select_distinct) {
              ...
              return collection.find(filters, {}, function(error, cursor) {
                return cursor.sort([[_sort_by, _sort_type]]).limit(limit).toArray(function(err, items) {
                  var i, to_return;
                  to_return = [];
